...
class AiInstance(QThread):
    finished = pyqtSignal(int)  # Signal now includes the process ID
...
class DirectorySelector(QWidget):
    ...
    def remove_instance(self, process_id, status):
        if process_id in self.aider_processes:
            instance = self.aider_processes[process_id]
            instance['widget'].setParent(None)  # Remove the widget from its parent layout
            instance['widget'].deleteLater()  # Schedule the widget for deletion
